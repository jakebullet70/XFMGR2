...
;
; ===========================================================================================
; VENDORED + PATCHED COPY (XFMGR2). This is a verbatim copy of prog8 v12.2.1's bundled
; prog8lib/cx16/diskio.p8, extracted from prog8c.jar, placed in SRC/ so it SHADOWS the library
; module for this project's whole build (main + every overlay). Changes from upstream:
;   (A) internal_next_entry() BOUNDS the directory-entry filename read. Upstream copies the name
;       into list_filename with no length check, so a filename longer than the buffer overruns it
;       and corrupts adjacent memory. The read is now capped at 51 chars (+ NUL), matching the
;       51-char RAM read cap elsewhere (see xfiles.NAME_RD_CAP / xarena.read_str).
;   (B) chdir/mkdir/rmdir build their "cd:"/"md:"/"rd:" DOS command INSIDE list_filename via an
;       unbounded strings.copy of the path. A path longer than the buffer overran it (the whole-
;       disk "Find" crawl chdir's into deep dirs whose paths reach ~60-100 chars) and corrupted
;       diskio state, crashing the NEXT diskio call (e.g. hist_load). They now use cmd_path_copy(),
;       a bounded copy, and list_filename is grown to 128 bytes so real paths fit without truncation.
; When upgrading prog8, RE-FORK: re-extract the bundled diskio.p8 and re-apply these patches.
; ===========================================================================================
...
io_error:
    ...
    str list_filename = "?" * 128       ; PATCHED: was 50; holds a listing name (capped 51) OR a
                                        ; "cd:"/"md:"/"rd:" command path (bounded by cmd_path_copy)
...
io_error:
    ...
    private sub internal_next_entry() -> bool {
        ...
        ; read the filename  (PATCHED: bounded so a name longer than list_filename can't overrun
        ; it - store at most 51 chars, but keep consuming to the closing quote to stay in sync
        ; with the channel; see the file header)
        ubyte fnlen = 0
        repeat {
...
+           lda  #1
    ...
    sub cmd_path_copy(str path) {
        ; PATCHED helper: copy `path` into list_filename starting at offset 3 (after the 2-char
        ; command + ':'), BOUNDED so a long path can't overrun the buffer (list_filename is 128).
        ubyte i = 0
        while path[i] != 0 and i < 123 {
            list_filename[3 + i] = path[i]
            i++
        }
        list_filename[3 + i] = 0
    }
