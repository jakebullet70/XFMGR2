...
#if defined(__OpenBSD__) || defined(__NetBSD__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__APPLE__) || defined(SVR4) || defined(__HAIKU__)
#include <locale.h>
#endif
...
extern int  InputString(char *prompt, char *s, int length, int  y, int cursor_pos, char *term);
...
extern int  GetCommandLine(char *command_line, int command_line_length);
extern int  GetSearchCommandLine(char *command_line, int command_line_length);
...
extern char *CutFilename(char *dest, char *src, unsigned int max_len, BOOL fill);
...
extern int  MvAddNStr(int y, int x, char *str, int n);
extern int  MvWAdNdStr(WINDOW *win, int y, int x, char *str, int n);
extern int  WAddNStr(WINDOW *win, char *str, int n);
extern int  AddNStr(char *str, int n);
